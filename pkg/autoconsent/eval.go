...
var JSEvals = map[string]string{
	"EVAL_0":                "return console.log(1);",
	...
	"EVAL_COINBASE_0": ` JSON.parse(decodeURIComponent(document.cookie.match(/cm_(eu|default)_preferences=([0-9a-zA-Z\\{\\}\\[\\]%:]*);?/)[2])).consent
            .length <= 1`,
	...
	"EVAL_MICROSOFT_0": `Array.from(document.querySelectorAll('div > button'))
            .filter((el) => el.innerText.match('Reject|Ablehnen'))[0]
            .click() || true`,
	"EVAL_MICROSOFT_1": `Array.from(document.querySelectorAll('div > button'))
	...
	"EVAL_PANDECTES_TEST": `document.cookie.includes('_pandectes_gdpr=') &&
        JSON.parse(
            atob(
                document.cookie
                    .split(';')
	...
	"EVAL_PUBTECH_0": `document.cookie.includes('euconsent-v2') &&
	...
	"EVAL_SHOPIFY_TEST": `document.cookie.includes('gdpr_cookie_consent=0') ||
